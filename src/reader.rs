...
impl<R: Read> Block<R> {
    fn new(reader: R) -> Result<Block<R>> {
        let mut block = Block {
            reader,
            codec: Codec::Null,
    ...
    fn read_header(&mut self) -> Result<()> {
        ...
        if buf != [b'O', b'b', b'j', 1u8] {
            return Err(error(ErrorKind::Decode("wrong magic in header".to_string())));
        }
        ...
        if let Value::Map(meta) = decode(&meta_schema, &mut self.reader)? {
            ...
            if let Some(schema) = schema {
                ...
                return Err(error(ErrorKind::SchemaParsing("unable to parse schema".to_string())));
            }
            ...
            return Err(error(ErrorKind::Decode("no metadata in header".to_string())));
        }
    ...
    fn fill_buf(&mut self, n: usize) -> Result<()> {
    ...
    fn read_block_next(&mut self) -> Result<()> {
        ...
        match util::read_long(&mut self.reader) {
            Ok(block_len) => {
                ...
                if marker != self.marker {
                    return Err(error(ErrorKind::Decode("block marker does not match header marker".to_string())));
                }
            ...
            Err(e) => {
                if let ErrorKind::Io(err) = e.kind() {
                    ...
                    if err.contains("unexpected end of file") || err.contains("failed to fill whole buffer") { //TODO horrible
                        return Ok(());
                    }
                }
            }
        };
        Err(error(ErrorKind::Decode("unable to read block".to_string())))
    }
    ...
    fn read_next(&mut self, read_schema: Option<&Schema>) -> Result<Option<Value>> {
        if self.is_empty() {
...
impl<'a, R: Read> Reader<'a, R> {
    ...
    pub fn new(reader: R) -> Result<Reader<'a, R>> {
    ...
    ///
    /// **NOTE** The avro header is going to be read automatically upon creation of the `Reader`.
    pub fn with_schema(schema: &'a Schema, reader: R) -> Result<Reader<'a, R>> {
        let block = Block::new(reader)?;
