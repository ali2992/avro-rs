...
pub fn zag_i32<R: Read>(reader: &mut R) -> Result<i32> {
    let i = zag_i64(reader)?;

    if i < i64::from(i32::min_value()) || i > i64::from(i32::max_value()) {
        Err(error(ErrorKind::Decode("int out of range".to_string())))
    } else {
        Ok(i as i32)
    }
}

pub fn zag_i64<R: Read>(reader: &mut R) -> Result<i64> {
...
fn decode_variable<R: Read>(reader: &mut R) -> Result<u64> {
    ...
    loop {
        if j > 9 {
            // if j * 7 > 64
            return Err(error(ErrorKind::Decode("Overflow when decoding integer value".to_string())));
        }
...
pub fn safe_len(len: usize) -> Result<usize> {
    let max_bytes = max_allocation_bytes(512 * 1024 * 1024);

    if len <= max_bytes {
        Ok(len)
    } else {
        Err(error(ErrorKind::Allocation(format!(
